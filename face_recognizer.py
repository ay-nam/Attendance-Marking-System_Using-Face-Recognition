...
def mark_attendance(collection, student_name):
    today_date = datetime.today().strftime('%Y-%m-%d')
    record = collection.find_one({'name': student_name, 'date': today_date})
    ...
    if not record:
        ...
        print(f"[INFO] Attendance marked for {student_name} on {today_date}")
    else:
        print(f"[INFO] Attendance already marked for {student_name} on {today_date}")
        ...
        for (x, y, w, h) in faces:
            ...
            if confidence > 51:
                try:
                    ...
                    # Mark attendance
                    mark_attendance(collection, name)
